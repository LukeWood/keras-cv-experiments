...
def visualize_detections(model, x, y, path):
    ...
    color = tf.constant(((255.0, 0, 0),))
    true_color = tf.constant(((0, 255.0, 0),))
    plt.figure(figsize=(10, 10))
    ...
    plotted_images = tf.image.draw_bounding_boxes(images, y[..., :4], true_color)
    for i in range(9):
        plt.subplot(9 // 3, 9 // 3, i + 1)
...
class VisualizeDetections(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs):
        visualize_detections(self.model, self.x, self.y, f"{self.artifacts_dir}/{epoch}.png")
